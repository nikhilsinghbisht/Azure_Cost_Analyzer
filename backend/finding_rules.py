"""finding_rules.py — deterministic cost/security rules over enriched resources."""
...
def _vm_cost_candidates(res: dict) -> list[dict]:
    ...
    # 3) Spot — non-prod only, and only if not already Spot.
    priority = (res.get("vm_priority") or "Regular").lower()
    if (
        spot and ondemand and spot < ondemand and nonprod
        and priority != "spot"
    ):
    ...
    # 4) Auto-shutdown — non-prod only, and only if not already enabled.
    if nonprod and res.get("auto_shutdown_enabled") is False:
        out.append(_issue(
            res,
            category="Cost Saving",
            ...
            current_config="Running 24/7 (168 h/week), auto-shutdown off",
...
def detect_issues(enriched_resources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    Returns a list of issue dicts (same schema as cost_analysis).
