"""resource_inventory.py — list/enrich Azure resources (REST or az CLI) + costs."""
    ...
    try:
        if rtype == "microsoft.compute/virtualmachines":
            ...
            try:
                ...
                # Spot vs Regular — don't recommend Spot if already Spot.
                extra["vm_priority"] = vm_props.get("priority") or "Regular"

                # Portal auto-shutdown = DevTestLab schedule on the VM.
                # 404 → not configured; Enabled → already on (skip recommendation).
                try:
                    vm_name = rid.rstrip("/").split("/")[-1]
                    rg_prefix = rid.rsplit("/providers/", 1)[0]
                    sched = _arm_get(
                        f"{rg_prefix}/providers/Microsoft.DevTestLab/schedules/"
                        f"shutdown-computevm-{vm_name}",
                        token,
                        "2018-09-15",
                    )
                    status = ((sched.get("properties") or {}).get("status") or "").lower()
                    extra["auto_shutdown_enabled"] = status == "enabled"
                except AzureResourceGroupNotFoundError:
                    extra["auto_shutdown_enabled"] = False
                except Exception:
                    extra["auto_shutdown_enabled"] = None
        ...
        # ── Azure Cache for Redis ────────────────────────────────────────────
            ...
            # True when non-SSL port is open (enableNonSslPort=true) — rule should flag that.
            extra["redis_non_ssl_enabled"] = bool(props.get("enableNonSslPort", False))
